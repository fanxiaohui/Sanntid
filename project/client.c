	...
	switch(message_receive.type){
		case ELEV_ORDER:
			...
			if(message_receive.ID==my_ID){ //Sets local queue based on ID
				...
				if(elevator.queue[message_receive.elevator.new_floor_order]==0){
	...
	while(1){
		...
		while(sum){
			...
			if(elevator.floor_current!=-1){
				...
				else if(elevator.queue[elevator.floor_current]==1){
					stop=1;
				}
				...
				if(stop){
					...
					switch (elevator.floor_current){//Updates correct order
						case 0:
				    ...
				    sum = 0;
				    switch (elevator.next_direction){//Checks for internal buttonpress in ordered direction
				    	case DIRN_DOWN:
				    		for(int i = elevator.floor_current;i>=0;i--){
				    			sum += elevator.queue[i];
				    		}

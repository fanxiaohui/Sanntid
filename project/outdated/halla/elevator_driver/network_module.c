...
//Lagre sockets i et array, itterer over for å oppnå multicast 
...
void *tcp_connection_handler(void *socket_desc) //Connection handler for TCP connections
{
    ...
    int read_size;
    char *message , client_message[BUFSIZE], halla[BUFSIZE];
    bzero(halla,BUFSIZE); 
    strcpy(halla,"halla");
...
void tcp_recieve(void *socket_desc){
  ...
  while(connection){
    ...
    if(!strcmp(buf,empty)){
	    ...
	    pthread_create(&message_handler_thread, NULL , message_handler, (void*)buf);	
	    }
    bzero(buf, BUFSIZE);
  }
  free(socket_desc);
...
/*void* elevator_thread(){
  ...
  while(1){
    if(elevator.new_floor_order!=-1){
     // printf("Going to floor: %d", elevator.new_floor_order);	

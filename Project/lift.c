	...
	while(1){
		if(sum != 0){
			...
			if(elevator.direction == DIRN_STOP){
				for(int i=elev.floor_current+1;i<N_FLOORS;i++){
			...
			while(sum){
				...
				if(elevator.queue[elevator.floor_current]==1){
					...
					serialization(ELEV_FLOOR_UPDATE, message_send, transmit);
...
Message deserialization(char *msg_recv){
	...
	message_receive.elevator.destination=msg_recv[3];
	message_receive.elevator.reached_destination=msg_recv[4];
